...
/usr/bin/autossh -M ${MONITOR_PORT} -NfR ${REMOTE_PORT}:localhost:${LOCAL_PORT} ${REMOTE_CONNECT}

nohup socat -s tcp4-listen:${LOCAL_PORT},reuseaddr,fork UDP:127.0.0.1:161 > /tmp/MagConsole.socat.out.log 2> /tmp/MagConsole.socat.error.log < /dev/null &
