#!/bin/sh
#也可以這樣子執行
# Usage: curl -s https://raw.githubusercontent.com/joetww/work_script/master/export_language.sh | bash /dev/stdin [<dev|pre|pro>] [useraccount]
display_usage() {
...
ssh ${TARGETUSER}@${TARGETIP} -t -p${TARGETPORT} "mkdir -p ~/language; sudo tar zcf ~/language/${1,,}_old_language_`date +\%Y\%m\%d_\%H\%M\%S`.tgz -C ${TARGETPATH} . --exclude-vcs && sudo tar zxvf - -C ${TARGETPATH} "
